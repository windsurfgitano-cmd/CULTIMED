...
async function createDispensation(formData: FormData) {
  ...
  transaction(() => {
    ...
    for (const it of items) {
      const b = get<{ q: number; pr: number }>(`SELECT quantity_current as q, price_per_unit as pr FROM batches WHERE id = ?`, it.batchId);
...
export default function NewDispensationPage({
  ...
  const prescriptions = all<RxLite>(
