...
async function updatePatient(formData: FormData) {
  ...
  if (!fullName) redirect(`/patients/${id}/edit?e=name`);
  if (!rutRaw) redirect(`/patients/${id}/edit?e=rut_missing`);
  if (!isValidRut(rutRaw)) redirect(`/patients/${id}/edit?e=rut_invalid`);
  const rut = formatRut(cleanRut(rutRaw));

  try {
    await run(
      `UPDATE patients
       SET rut = ?, full_name = ?, date_of_birth = ?, gender = ?,
      ...
      ecName, ecPhone, allergies, conditions, notes, membershipStatus, id
    );
    await logAudit({
      staffId: staff.id,
    ...
    redirect(`/patients/${id}?ok=updated`);
  } catch (err: any) {
    if (String(err).includes("UNIQUE")) {
      redirect(`/patients/${id}/edit?e=duplicate`);
    }
    throw err;
  }
}
...
const ERR_MSG: Record<string, string> = {
  ...
  duplicate: "Ya existe otro paciente con ese RUT.",
};
...
export default async function EditPatientPage({
  ...
  return (
    <>
      <PageHeader
        title={`Editar paciente: ${p.full_name}`}
        subtitle="Modifica los datos del socio. Cambios quedan registrados en bitácora de auditoría."
        actions={
          <Link href={`/patients/${id}`} className="btn-secondary">
      ...
      <form action={updatePatient} className="space-y-6">
        ...
        <Section title="Identificación" icon="badge">
          <Field label="RUT *" name="rut" defaultValue={p.rut} required />
          <Field label="Nombre completo *" name="full_name" defaultValue={p.full_name} required colSpan={2} />
          <Field label="Fecha de nacimiento" name="date_of_birth" type="date" defaultValue={p.date_of_birth?.split("T")[0]} />
          <SelectField
            label="Género"
            name="gender"
        ...
        <Section title="Contacto" icon="contact_mail">
          ...
          <Field label="Teléfono" name="phone" type="tel" defaultValue={p.phone || ""} placeholder="+56 9 XXXX XXXX" />
          <Field label="Dirección" name="address" defaultValue={p.address || ""} colSpan={2} />
          <Field label="Ciudad / Comuna" name="city" defaultValue={p.city || ""} />
        </Section>

        <Section title="Contacto de emergencia" icon="emergency">
          <Field label="Nombre" name="emergency_contact_name" defaultValue={p.emergency_contact_name || ""} />
          <Field label="Teléfono" name="emergency_contact_phone" type="tel" defaultValue={p.emergency_contact_phone || ""} />
        </Section>

        <Section title="Información clínica" icon="medical_information">
          <Field label="Alergias" name="allergies" defaultValue={p.allergies || ""} colSpan={2} />
          <Field label="Condiciones crónicas" name="chronic_conditions" defaultValue={p.chronic_conditions || ""} colSpan={2} />
          <Field label="Notas adicionales" name="notes" defaultValue={p.notes || ""} colSpan={2} />
        </Section>
        ...
        <Section title="Estado de membresía" icon="verified_user">
          <SelectField
            ...
            defaultValue={p.membership_status}
            options={[
              { v: "active", l: "Activo" },
...
function Field({
  label, name, type = "text", required = false, placeholder, defaultValue, colSpan,
}: {
  label: string; name: string; type?: string; required?: boolean; placeholder?: string; defaultValue?: string; colSpan?: number;
}) {
  return (
    <div className={colSpan === 2 ? "md:col-span-2" : ""}>
      <label className="input-label" htmlFor={name}>{label}</label>
      <input id={name} name={name} type={type} required={required} placeholder={placeholder} defaultValue={defaultValue} className="input-field" />
    </div>
  );
}
